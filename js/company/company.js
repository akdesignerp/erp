...
/* ===== ERP REBUILD COMPANY AUTH ===== */
function setCompanyAuthStatus(msg, isErr){
  const box = document.getElementById("companyAuthStatus");
  if (!box) return;
  box.textContent = msg;
...
async function getCompanyPasswordRow(){
  ...
  const { data, error } = await db.from("company_admin_auth").select("*").limit(1).maybeSingle();
  if (error) throw error;
  return data;
}
function requestCompanyAccess(){
  document.getElementById("companyAuthModal")?.classList.add("show");
  document.getElementById("companyAuthPassword").value = "";
  document.getElementById("companyOldPassword").value = "";
  document.getElementById("companyNewPassword").value = "";
  closeCompanyPasswordChangeView();
  setCompanyAuthStatus("회사관리 비밀번호를 입력하세요.");
}
function closeCompanyAuthModal(){ document.getElementById("companyAuthModal")?.classList.remove("show"); }
function openCompanyPasswordChangeView(){
  document.getElementById("companyAuthLoginView")?.classList.add("hidden");
  document.getElementById("companyAuthChangeView")?.classList.remove("hidden");
  setCompanyAuthStatus("기존 비밀번호 확인 후 새 비밀번호를 저장합니다.");
}
function closeCompanyPasswordChangeView(){
  document.getElementById("companyAuthChangeView")?.classList.add("hidden");
  document.getElementById("companyAuthLoginView")?.classList.remove("hidden");
}
async function submitCompanyAuth(){
  try {
    const row = await getCompanyPasswordRow();
    if (!row) throw new Error("company_admin_auth 테이블에 비밀번호가 없습니다.");
    const input = document.getElementById("companyAuthPassword")?.value || "";
    ...
    document.querySelectorAll(".tab-btn").forEach(btn => btn.classList.toggle("active", btn.dataset.tab === "company"));
...
async function changeCompanyPassword(){
  try {
    const row = await getCompanyPasswordRow();
    if (!row) throw new Error("기존 비밀번호 정보가 없습니다.");
    const oldPw = document.getElementById("companyOldPassword")?.value || "";
    const newPw = document.getElementById("companyNewPassword")?.value || "";
    ...
    const result = await db.from("company_admin_auth").update({ password_plain: newPw }).eq("id", row.id);
...
document.addEventListener("DOMContentLoaded", () => {
  [["companyAuthPassword", submitCompanyAuth],["companyOldPassword", changeCompanyPassword],["companyNewPassword", changeCompanyPassword]].forEach(([id, fn]) => {
    const el = document.getElementById(id);
...
/* ===== 2026-04-18 company auth patch ===== */
function openCompanyTabDirectly(){
  ["counsel","write","summary","detail","contract","company"].forEach(name => {
...
submitCompanyAuth = async function(){
  try {
    const row = await getCompanyPasswordRow();
    if (!row) throw new Error("company_admin_auth 테이블에 비밀번호가 없습니다.");
    const input = document.getElementById("companyAuthPassword")?.value || "";
    if (!input) return setCompanyAuthStatus("비밀번호를 입력하세요.", true);
    const saved = String(row.password_plain || row.password || "").trim();
    if (saved !== String(input).trim()) return setCompanyAuthStatus("비밀번호가 일치하지 않습니다.", true);
    closeCompanyAuthModal();
    openCompanyTabDirectly();
  } catch (err) {
    console.error(err);
    setCompanyAuthStatus("회사관리 인증 실패: " + err.message, true);
  }
